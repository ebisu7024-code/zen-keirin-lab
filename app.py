...
except ImportError:  # pragma: no cover - SQLiteだけで動かす環境を残す
    psycopg = None
    dict_row = None
...
class PostgresConnection:
    ...
    def __init__(self, database_url: str):
        if psycopg is None:
            raise RuntimeError("PostgreSQL接続には psycopg が必要です。`pip install -r requirements.txt` を実行してください。")
    ...
    def execute(self, sql: str, params: Sequence[Any] | Mapping[str, Any] | None = None) -> PostgresCursor:
        has_params = params is not None and not (isinstance(params, Sequence) and not isinstance(params, str) and len(params) == 0)
        cursor = self.raw.execute(translate_sql_for_postgres(sql, has_params=has_params), params if has_params else None)
        return PostgresCursor(cursor)

    def executemany(self, sql: str, params_seq: Sequence[Sequence[Any]]) -> PostgresCursor:
        cursor = self.raw.cursor()
        cursor.executemany(translate_sql_for_postgres(sql, has_params=True), params_seq)
        return PostgresCursor(cursor)
...
def database_backend() -> str:
    database_url = configured_database_url().lower()
...
def get_conn():
    if database_backend() == "postgres":
        return PostgresConnection(configured_database_url())
...
def translate_sql_for_postgres(sql: str, *, has_params: bool = False) -> str:
    ...
    placeholder_token = "__ZEN_KEIRIN_SQL_PARAM__"
    translated = translated.replace("?", placeholder_token)
    if has_params:
        translated = translated.replace("%", "%%")
    translated = translated.replace(placeholder_token, "%s")
...
def safe_identifier(identifier: str) -> str:
    if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", identifier):
        raise ValueError(f"安全でない識別子です: {identifier}")
    return identifier
...
def sqlite_table_columns(conn: sqlite3.Connection, table: str) -> list[str]:
    return [row["name"] for row in conn.execute(f"PRAGMA table_info({safe_identifier(table)})")]
...
def table_primary_key_columns(conn, table: str) -> tuple[str, ...]:
    ...
    return tuple(
        row["name"]
        for row in sorted(
            (row for row in conn.execute(f"PRAGMA table_info({table})") if int(row["pk"] or 0) > 0),
            key=lambda row: int(row["pk"]),
        )
    )


def list_active_table_names(conn) -> set[str]:
    if is_postgres_connection(conn):
        rows = conn.execute(
            """
...
def ensure_column(conn, table: str, column: str, definition: str) -> None:
    ...
    if column not in columns:
        conn.execute(f"ALTER TABLE {safe_identifier(table)} ADD COLUMN {safe_identifier(column)} {definition}")


def backfill_source_race_ids(conn) -> None:
    rows = conn.execute(
        """
        SELECT id, race_memo
        FROM races
...
def migrate_race_result_rows_primary_key(conn) -> None:
    pk_columns = table_primary_key_columns(conn, "race_result_rows")
...
def enable_postgres_row_level_security(conn) -> None:
...
def database_counts(db_path: Path | None = None) -> dict[str, int]:
    if db_path is not None:
        return count_tables_in_sqlite_file(db_path)
    if database_backend() == "postgres":
        with get_conn() as conn:
            table_names = list_active_table_names(conn)
            counts: dict[str, int] = {}
            for table in REQUIRED_DB_TABLES:
                if table in table_names:
...
def reset_postgres_identity_sequences(conn) -> None:
    for table in ("races", "riders", "bets", "race_payouts", "race_lines"):
        conn.execute(
            f"""
            SELECT setval(
...
def import_sqlite_database_to_active_db(sqlite_path: Path) -> dict[str, int]:
    if database_backend() != "postgres":
        raise ValueError("SQLiteファイルのクラウドDB取込はPostgreSQL接続時だけ使えます。")
    ...
    with sqlite3.connect(sqlite_path) as source_conn, get_conn() as dest_conn:
        ...
        for table in REQUIRED_DB_TABLES:
            ...
            if not columns:
                continue
            column_list = ", ".join(quoted_identifier(column) for column in columns)
            rows = source_conn.execute(f"SELECT {column_list} FROM {safe_identifier(table)}").fetchall()
            if not rows:
                continue
        ...
        reset_postgres_identity_sequences(dest_conn)
...
def restore_database(uploaded_file) -> dict[str, int]:
    ...
    if database_backend() == "postgres":
        try:
            return import_sqlite_database_to_active_db(upload_path)
        finally:
...
def row_to_dict(row: sqlite3.Row | Mapping[str, Any] | None) -> dict:
    return dict(row) if row else {}


def cursor_column_names(description) -> list[str]:
    if not description:
        return []
    return [getattr(column, "name", column[0]) for column in description]


def read_sql_query(query: str, conn, params: Sequence[Any] | Mapping[str, Any] | None = None) -> pd.DataFrame:
    if is_postgres_connection(conn):
        cursor = conn.execute(query, params or ())
        columns = cursor_column_names(cursor.description)
...
def returning_id_sql(conn) -> str:
    return " RETURNING id" if is_postgres_connection(conn) else ""
...
def inserted_id(cursor) -> int:
    ...
    row = cursor.fetchone()
    if row is None:
        raise RuntimeError("INSERT後のIDを取得できませんでした。")
...
def fetch_rider_result_details() -> pd.DataFrame:
    ...
    """
    with get_conn() as conn:
        details = read_sql_query(query, conn)
    ...
    with get_conn() as conn:
        if race_id:
            ...
            cursor = conn.execute(
                ...
                {returning_id_sql(conn)}
                """,
                [*values, timestamp, timestamp],
            )
            saved_id = inserted_id(cursor)
            if source_race_id:
...
def upsert_winticket_race_listing(listing) -> tuple[int, bool]:
    ...
    with get_conn() as conn:
        ...
        return inserted_id(cursor), True
...
def render_database_admin_panel() -> None:
    with st.sidebar.expander("DBバックアップ/復元"):
        st.caption(f"接続先: {database_backend_label()}")
        ...
        if counts:
            ...
            if database_backend() == "sqlite":
                ...
                st.caption("クラウドDB接続中です。SQLiteファイルのダウンロードではなく、Supabase側を正本として使います。")
        else:
            st.caption("現在のDBはまだ作成されていません。")

        restore_label = "SQLite DBをクラウドDBへ取り込む" if database_backend() == "postgres" else "復元するSQLite DBをアップロード"
        ...
        if uploaded_db is not None:
            ...
            if database_backend() == "postgres":
                st.caption("アップロード後、自動でクラウドDBを上書き取込します。")
            else:
